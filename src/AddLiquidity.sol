...
contract AddLiquidity {
    ...
    function addLiquidity(address rad, address usdc) public {
        require(
            ERC20(rad).transferFrom(msg.sender, address(this), radLiquidity),
            "Transfer of RAD must succeed"
        );
        require(
            ERC20(usdc).transferFrom(msg.sender, address(this), usdcLiquidity),
            "Transfer of USDC must succeed"
        );
        ERC20(rad).approve(UNISWAP_ROUTER, radLiquidity);
        ERC20(usdc).approve(UNISWAP_ROUTER, usdcLiquidity);
